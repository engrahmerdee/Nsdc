...
# ─── Simple obfuscation replacing Fernet (no cryptography package needed) ─────
# Uses base64 so sensitive fields are not stored as plaintext.
# For true encryption, set FERNET_KEY and add cryptography back to requirements.
import base64
...
class User(UserMixin, db.Model):
    ...
    posts               = db.relationship('Post', backref='author', lazy='select', foreign_keys='Post.user_id')
    comments            = db.relationship('Comment', backref='author', lazy='select')
    votes               = db.relationship('Vote', backref='voter', lazy='select')
    sent_requests       = db.relationship('FriendRequest', foreign_keys='FriendRequest.sender_id', backref='sender', lazy='select')
    received_requests   = db.relationship('FriendRequest', foreign_keys='FriendRequest.receiver_id', backref='receiver', lazy='select')
    sent_messages       = db.relationship('PrivateMessage', foreign_keys='PrivateMessage.sender_id', backref='sender', lazy='select')
    notifications       = db.relationship('Notification', foreign_keys='Notification.user_id', backref='user', lazy='select')
    reports_made        = db.relationship('Report', foreign_keys='Report.reporter_id', backref='reporter', lazy='select')
    activity_logs       = db.relationship('AdminLog', foreign_keys='AdminLog.actor_id', backref='actor', lazy='select')
    group_invites_sent  = db.relationship('GroupInvite', foreign_keys='GroupInvite.invited_by', backref='inviter', lazy='select')
...
class Group(db.Model):
    ...
    invites      = db.relationship('GroupInvite', backref='group', lazy='select', cascade='all, delete-orphan')

    def member_count(self):
        from sqlalchemy import select, func
        return db.session.execute(
            select(func.count()).where(group_members.c.group_id == self.id)
        ).scalar()
...
class Poll(db.Model):
    ...
    votes       = db.relationship('Vote', backref='poll', lazy='select', cascade='all, delete-orphan')
...
class PollOption(db.Model):
    ...
    def vote_count(self):
        return Vote.query.filter_by(option_id=self.id).count()
