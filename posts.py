...
from models import db, Post, Comment, Like, Notification, User, AdminLog, Report
...
def submit():
    if not current_user.can_post:
        flash('Only verified leaders can submit posts for review.', 'warning')
    ...
    if request.method == 'POST':
        ...
        # Admin posts are auto-approved; others go to review
        ...
        if status == 'approved' and is_announcement:
            # Notify all users
            for u in User.query.filter(User.is_suspended == False, User.id != current_user.id).all():
                push_notification(u.id, 'announcement',
                    f'📢 New announcement: {title or content[:60]}',
        ...
        if status == 'pending':
            ...
            db.session.commit()
            flash('Post submitted for review. Admin will approve it shortly.', 'info')
        else:
...
def report_post(post_id):
    ...
    db.session.add(r)
    db.session.commit()
    flash('Post reported. Admin will review it.', 'info')
    return redirect(url_for('posts.view', post_id=post_id))
