...
def dashboard():
    ...
    # Posts pending MY review (only admin)
    pending_posts_count = 0
    if current_user.role == 'admin':
        pending_posts_count = Post.query.filter_by(status='pending').count()

    # People you may know (same school)
    blocked_ids = [b.blocked_id for b in Block.query.filter_by(blocker_id=current_user.id)]
    blocked_ids += [b.blocker_id for b in Block.query.filter_by(blocked_id=current_user.id)]
