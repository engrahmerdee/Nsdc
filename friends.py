...
def accept(req_id):
    ...
    if sender:
        ...
        push_notification(req.sender_id, 'friend_request',
            ...
            url_for('auth.profile', user_id=current_user.id))
    db.session.commit()
    flash(f'You are now connected with {sender.full_name.split()[0] if sender else "them"}!', 'success')
...
def block(user_id):
    ...
    if not Block.query.filter_by(blocker_id=current_user.id, blocked_id=user_id).first():
        ...
        # Remove friendship if exists — use db.session.get() instead of Query.get()
...
@login_required
def list_friends():
    # lazy='select': friends is now a plain list — filter in Python
    friends = [f for f in current_user.friends if not f.is_suspended]
