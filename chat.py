"""
NSDCP Chat — Private & Group Messaging
SA2 compatible — fixed .update() and .get()
"""
...
def inbox():
    ...
    for fid in friend_ids:
        ...
        if not friend:
            continue
        last = db.session.query(PrivateMessage).filter(
            db.or_(
                db.and_(PrivateMessage.sender_id == current_user.id,
                        PrivateMessage.receiver_id == fid),
                db.and_(PrivateMessage.sender_id == fid,
                        PrivateMessage.receiver_id == current_user.id)
            )
        ).order_by(PrivateMessage.created_at.desc()).first()
        unread = db.session.query(PrivateMessage).filter_by(
...
def conversation(user_id):
    ...
    blocked = Block.query.filter(
        db.or_(
            db.and_(Block.blocker_id == current_user.id, Block.blocked_id == user_id),
            db.and_(Block.blocker_id == user_id,         Block.blocked_id == current_user.id)
        )
    ).first()
    if blocked:
    ...
    # Mark as read — SA2 compatible
    db.session.query(PrivateMessage).filter_by(
    ...
    messages = db.session.query(PrivateMessage).filter(
        db.or_(
            ...
            db.and_(PrivateMessage.sender_id == user_id,
                    PrivateMessage.receiver_id == current_user.id)
        )
...
def on_pm_send(data):
    ...
    emit('pm_receive', {
        ...
        'sender_id': cu.id, 'sender_name': cu.display_name,
