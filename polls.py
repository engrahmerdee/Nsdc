...
def create():
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        flash('Poll created and users notified!', 'success')
...
def vote(poll_id):
    ...
    if existing:
        if existing.is_locked:
            flash('Your vote is permanently locked (max changes reached).', 'warning')
            return redirect(url_for('polls.view', poll_id=poll_id))
        ...
        if existing.change_count >= MAX_CHANGES:
            ...
            flash(f'Vote updated and now permanently locked (used all {MAX_CHANGES} changes).', 'warning')
        else:
            remaining = MAX_CHANGES - existing.change_count
            flash(f'Vote changed. {remaining} change{"s" if remaining != 1 else ""} remaining.', 'info')
    else:
